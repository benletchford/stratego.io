...
from .gae import GAEBackend
from .util import ensure_text, validate_channel, validate_socket_id, app_id_re, pusher_url_re, channel_name_re
...
class Pusher(object):
    ...
    def __init__(self, app_id, key, secret, ssl=True, host=None, port=None, timeout=5, cluster=None,
                 json_encoder=None, json_decoder=None, backend=GAEBackend, **backend_options):
        ...
        if host:
            ...
            self._host = six.text_type(
                "api-%s.pusher.com") % ensure_text(cluster, "cluster")
        else:
    ...
    def authenticate(self, channel, socket_id, custom_data=None):
        ...
        if not channel_name_re.match(channel):
            raise ValueError(
                'Channel should be a valid channel, got: %s' % channel)
